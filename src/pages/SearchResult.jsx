...
function SearchResult() {
  ...
  getSearch();
    ...
    const getSearch=async()=>{
      const SearchAPI= await fetch(`https://api.themoviedb.org/3/search/multi?api_key=${process.env.REACT_APP_MOVIE_API}&language=en-US&query=${params.search}&include_adult=false`)
    ...
    if (params.search==="undefined"||Searcher.length===0) {
      return(
        <div className='search_result column_top_center'>
          <h1>There is no result related with your searching</h1>
          <div className='bottom_button_cover center_center'>
